...
export class Game {
  ...
  private lastWaveScoreDetails: ReturnType<ScoreManager['finalizeWave']> | null = null;
  ...
  constructor(canvas: HTMLCanvasElement) {
    ...
    this.waveManager = new WaveManager(
      (kind: EnemyKind, spawnIdx: number) => { this.spawnEnemy(kind, spawnIdx); },
      (waveNumber: number)               => { this.onWaveClear(waveNumber); },
    );
    ...
    this.inputHandler = new InputHandler(canvas, this.renderer, this.mapGrid, {
      ...
      onEggPlaced:     (col, row) => { this.handleEggPlaced(col, row); },
      onBuildComplete: (spot)     => { this.placeAtSpot(spot); },
      onPlayerMove:    (x, y)     => { this.player.setTarget(x, y); },
      onAttack:        ()         => {
  ...
  private spawnEnemy(kind: EnemyKind, spawnIdx: number): void {
    ...
    for (const gi of this.activeGoalIndices) {
      ...
      const cells = findPath(spawnCell, goalCell, (c, r) => this.mapGrid.isPathTile(c, r));
  ...
  private redirectEnemyToNextGoal(enemy: Enemy): void {
    ...
    for (const gi of this.activeGoalIndices) {
      ...
      if (this.goalEggs[gi] <= 0) continue;
      const goalCell = { col: GOAL_COLS[gi], row: GOAL_ROW };
      const cells = findPath(startCell, goalCell, (c, r) => this.mapGrid.isPathTile(c, r));
      if (cells && cells.length < bestLen) {
        bestLen = cells.length;
        bestWaypoints = cells.map(cell => this.mapGrid.tileCenter(cell.col, cell.row));
        bestGoalIdx = gi;
  ...
  private drawSpotEntity(spot: PlacementSpot): void {
    switch (spot.kind) {
      case 'octopus':    createTower(TowerKind.Octopus,    spot.col, spot.row).draw(this.renderer); break;
      case 'crab':       createTower(TowerKind.Crab,       spot.col, spot.row).draw(this.renderer); break;
      case 'hermit_crab':createTower(TowerKind.HermitCrab, spot.col, spot.row).draw(this.renderer); break;
      case 'eel':        createTower(TowerKind.Eel,        spot.col, spot.row).draw(this.renderer); break;
      case 'coral':      new CoralWall(spot.col, spot.row).draw(this.renderer); break;
    }
  }
  ...
  private drawHoldPreview(): void {
    ...
    ctx.save();
    ctx.beginPath();
    ctx.arc(center.x, center.y, 26, -Math.PI / 2, -Math.PI / 2 + Math.PI * 2 * progress);
