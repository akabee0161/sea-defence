...
import { ENEMY_DEFS, type EnemyKind } from '../defs/enemyDefs.ts';
...
export class Enemy extends GameObject {
  ...
  constructor(waypoints: Vector2D[], kind: EnemyKind = 'small', goalIdx = 0) {
    ...
    if (!def)
      throw new Error(`Unknown enemy kind: "${kind}". Add it to ENEMY_DEFS.`);
  ...
  private drawShark(renderer: Renderer): void {
    ...
    const jawDrop = this.isAtGoal
      ? r * (0.1 + 0.14 * Math.abs(Math.sin(Date.now() / 280)))
      : r * 0.32;
    const jawBack = this.isAtGoal ? jawDrop * 0.8 : r * 0.26;

    ctx.beginPath();
    ctx.moveTo(r * 0.82, -r * 0.05);
    ctx.lineTo(r * 1.28, -r * 0.08);
    ctx.lineTo(r * 1.2, jawDrop);
