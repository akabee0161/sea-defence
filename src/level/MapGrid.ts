...
export type { PlacementSpot, SpotKind } from '../defs/mapDefs.ts';
...
const SPAWN_CELLS: ReadonlyArray<{
  readonly col: number;
  readonly row: number;
...
export class MapGrid {
  ...
  constructor() {
    this.paths = MAP_DEF.paths.map((wp) =>
      wp.map(([c, r]) => tileCenter(c, r)),
    );
  ...
  get allSpots(): readonly PlacementSpot[] {
    return MAP_DEF.placementSpots as readonly PlacementSpot[];
  }
  ...
  tileCenter(col: number, row: number): Vector2D {
    return tileCenter(col, row);
  }

  spotCenter(col: number, row: number): Vector2D {
    return tileCenter(col, row);
  }
  ...
  drawGoalMarkers(
    ...
    for (const gi of activeGoalIndices) {
      ...
      glow.addColorStop(
        0,
        count > 0 ? 'rgba(255, 200, 80, 0.35)' : 'rgba(150, 150, 150, 0.25)',
      );
